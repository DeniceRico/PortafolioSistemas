...
function actualizarTablas() {
  mostrarLote(); // Actualiza la tabla de lotes
  actualizarTablaListos(); // Actualiza la tabla de listos
  actualizarTablaEjecucion(); // Actualiza la tabla de ejecución
  actualizarTablaBloqueados(); // Actualiza la tabla de bloqueados
  actualizarTablaTerminados(); // Actualiza la tabla de terminados
}


// Actualización para agregar tiempoServicio
function agregarProceso() {
  if (lotes.length > 0) { // Verifica si hay procesos en la cola de 'nuevos'
    const totalProcesosEnMemoria = loteEnEjecucion.length + procesosBloqueados.length + (procesoEnEjecucion ? 1 : 0);

    if (totalProcesosEnMemoria < 5) {
      const nuevoProceso = lotes.shift(); // Sacar el primer proceso de 'nuevos'
      loteEnEjecucion.push(nuevoProceso); // Agregar a listos
      console.log(`Proceso agregado: ${nuevoProceso.id}`); // Añade un log para verificar
    } else {
      console.log("Memoria llena. El proceso se queda en 'Nuevos'.");
    }

    actualizarTablas();
  } else {
    console.log("No hay procesos en 'Nuevos'");
  }
}
...
function tabla() {
  ...
  procesoBloqueado.forEach((proceso) => {
    ...
    nuevaFila.insertCell(9).textContent = `Tiempo bloqueado restante: ${proceso.tiempoBloqueadoRestante}`;
  ...
  procesosTerminados.forEach((proceso) => {
    ...
    nuevaFila.insertCell(6).textContent = proceso.tiempoDeInicio - proceso.tiempoDeLlegada;
    nuevaFila.insertCell(7).textContent = proceso.tiempoDeFinalizacion - proceso.tiempoDeInicio;
    nuevaFila.insertCell(8).textContent = proceso.tiempoDeFinalizacion - proceso.tiempoDeLlegada;
...
function mostrarProcesosPendientes() {
  ...
  lotes.forEach((lote) => {
    lote.forEach((proceso) => {
      ...
      nuevaFila.insertCell(9).textContent = "NULL"; // Estado bloqueado
    });
  });
...
function mostrarLote() {
  ...
  if (loteEnEjecucion.length === 0) {
    ...
    loteEnEjecucion.forEach((proceso) => {
      ...
      nuevaFila.insertCell(9).textContent = "Listo"; // Estado listo
